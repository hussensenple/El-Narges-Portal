...
const RoleChangeModal = ({ userId, userName, currentRole, onClose, onSuccess }: RoleChangeModalProps) => {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
      <div style={{ backgroundColor: '#161b22', padding: '30px', borderRadius: '10px', width: '400px', border: '1px solid #30363d' }}>
        <h3 style={{ marginTop: 0, color: '#58a6ff' }}>Change Role for {userName}</h3>
        
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '5px', color: '#c9d1d9' }}>Target Role</label>
            <select 
              value={targetRole} 
              onChange={(e) => setTargetRole(e.target.value)}
              style={{ width: '100%', padding: '10px', backgroundColor: '#0d1117', color: '#fff', border: '1px solid #30363d', borderRadius: '5px' }}
            >
          ...
          {['broker', 'engineer', 'admin'].includes(targetRole) && (
            <div>
              <label style={{ display: 'block', marginBottom: '5px', color: '#c9d1d9' }}>Manual ID (Required)</label>
              <input required type="text" value={manualId} onChange={(e) => setManualId(e.target.value)} style={{ width: '100%', padding: '10px', backgroundColor: '#0d1117', color: '#fff', border: '1px solid #30363d', borderRadius: '5px' }} />
            </div>
          )}

          {['engineer', 'admin'].includes(targetRole) && (
            <div>
              <label style={{ display: 'block', marginBottom: '5px', color: '#c9d1d9' }}>Age</label>
              <input type="number" value={age} onChange={(e) => setAge(e.target.value)} style={{ width: '100%', padding: '10px', backgroundColor: '#0d1117', color: '#fff', border: '1px solid #30363d', borderRadius: '5px' }} />
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
            ...
            <button type="submit" disabled={loading} style={{ flex: 1, padding: '10px', backgroundColor: '#238636', color: '#fff', border: 'none', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold' }}>
