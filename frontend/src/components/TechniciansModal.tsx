                ...
                {isDropdownOpen && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: '8px', backgroundColor: '#161b22', border: '1px solid #30363d', borderRadius: '8px', zIndex: 10, maxHeight: '300px', overflowY: 'auto', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }}>
                    <div
                      ...
                      onMouseLeave={(e) => e.currentTarget.style.backgroundColor = selectedSpecialization === 'All' ? '#1f242c' : 'transparent'}
                    >
                      All Specialists
                    </div>
                    {sortedSpecializations.map(spec => (
                      <div
                        ...
                        onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#1f242c'}
                        onMouseLeave={(e) => e.currentTarget.style.backgroundColor = selectedSpecialization === spec ? '#1f242c' : 'transparent'}
                      >
                        <span>{spec}</span>
                        {complaintCounts[spec] > 0 && (
                          <span style={{ backgroundColor: '#da3633', color: '#fff', padding: '2px 8px', borderRadius: '12px', fontSize: '11px', fontWeight: 'bold', boxShadow: '0 0 10px rgba(218,54,51,0.4)' }}>
                      ...
                      return (
                        <div key={spec} style={{ backgroundColor: '#161b22', padding: '20px', borderRadius: '12px', border: '1px solid #30363d' }}>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '10px' }}>
                            {group.map(tech => (
                              <div key={tech._id} style={{ backgroundColor: '#0d1117', border: '1px solid #30363d', borderRadius: '8px', padding: '10px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                  <span style={{ color: '#fff', fontWeight: 'bold', fontSize: '13px' }}>{tech.name}</span>
                                  <span style={{ backgroundColor: tech.status === 'Active' ? '#2ea04333' : '#da363333', color: tech.status === 'Active' ? '#3fb950' : '#ff7b72', padding: '2px 6px', borderRadius: '12px', fontSize: '10px', fontWeight: 'bold' }}>
                                    {tech.status}
                                  </span>
                                </div>
                                <div style={{ color: '#8b949e', fontSize: '11px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                  📞 <a href={`tel:${tech.phone}`} style={{ color: '#58a6ff', textDecoration: 'none' }}>{tech.phone}</a>
                                </div>
                                <div style={{ color: '#8b949e', fontSize: '11px' }}>🎂 {tech.age} Years Old</div>
                                <div style={{ color: '#f85149', fontSize: '11px', fontWeight: 'bold', marginTop: '2px' }}>
