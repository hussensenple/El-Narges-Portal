...
- **Show All Issues Red Dots Fix (August 2026):** Fixed `ShowAllIssuesButton` (`clearAll()`) to also remove dynamically-named external complaint dot layers (IDs prefixed with `ext-dot-`). Previously only `"complaint-marker-layer"` and `"all-issues-layer"` were removed, leaving external complaint red dots permanently on the map after toggling off.
- **Engineer Portal Chat Blank Page Fix (August 2026):** Fixed the `ComplaintChatModal` call inside `EngineerPortalModal` — the prop was incorrectly named `onUpdate` (should be `onRefresh`) and the required `currentUser` prop was missing entirely, causing the chat modal to render as a blank page.
- **Engineer Chatbot Error Fix (August 2026):** Fixed `askEngineerAI` backend controller — JSON parsing was fragile (direct `JSON.parse` without regex extraction), causing a 500 crash when Gemini wrapped responses in markdown fences. Also added `503 Service Unavailable` retry support to `generateWithRetry` (alongside existing `429`), and changed the error catch to return HTTP `200` with a friendly message instead of `500`, so the frontend displays it as a chat bubble instead of a crash.
...
- `RejectionAnalysisTab.tsx` — Two-panel view for analyzing rejection reasons (filterable list + live recharts bar chart that is fully responsive and dynamically synced with the selected rejection source)
