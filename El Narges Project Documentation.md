...
### 4. خريطة التوزيع الجغرافي للعملاء (Choropleth Web Map)
تم إضافة نظام ديناميكي لتحليل أماكن تواجد العملاء والملاك بناءً على حقل `governorate` الذي يتم اختياره أثناء التسجيل.
- يتم جلب إحصائيات التوزيع الجغرافي عبر Endpoint مخصصة `/api/admin/regions-stats`.
- تظهر النتائج في لوحة تحكم الإدارة كقائمة للمحافظات الأكثر مبيعاً أو تفاعلاً.
- يمكن للمدير النقر على أيقونة الخريطة لفتح `RegionsWebMapModal`، والذي يعرض خريطة ويب (Web Map) تفاعلية من ArcGIS.
- تقوم الخريطة بتلوين المحافظات بنظام (Equal Intervals) لدرجات اللون الأزرق الداكن بناءً على عدد العملاء في كل محافظة (Choropleth Renderer)، مع توفير تفاصيل دقيقة عن الملاك في كل منطقة عند النقر.
