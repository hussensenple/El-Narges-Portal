...
- **تحديث المساعد الذكي للمهندسين (Engineer Chatbot Widget):** تم توحيد واجهة الشات بوت الخاصة بالمهندس لتشمل جميع المميزات المتقدمة (Sidebar, Chat History, Pin, Rename, Delete) لضمان تجربة مستخدم متسقة مع باقي المنصة، مع إصلاح الألوان لتعمل بكفاءة عالية في كلاً من الوضعين الليلي والنهاري.
- **استقلالية وضع الرؤية (Theme Independence):** تم فصل حالة (Light/Dark Mode) بشكل كامل بين صفحة الـ Admin Dashboard وصفحة الـ Customer Portal لضمان عدم تأثر إعدادات الزوار أو المهندسين أو الملاك بما يختاره مدير النظام.

## 12. إصلاح ظهور الطلبات على الوحدات غير المخصصة (Unassigned Units Fix)
تم تعديل الكود لضمان عدم ظهور أو إتاحة طلب أي وحدة (شقة أو فيلا) للعميل في الكتالوج (`UnitCatalog.tsx`) أو في الـ `BuildingSidebar.tsx` إلا إذا كانت هذه الوحدة مخصصة لـ Broker. تم برمجة API جديدة `GET /api/roles/assigned-units` لجلب الوحدات المخصصة، وتمت فلترة الكتالوج الرئيسي `mode=all` ليعرض فقط هذه الوحدات.

## 13. تحسينات عامة وتنظيف (Enhancements & Cleanup)
- تم تعديل أسماء الـ Dashboards لتصبح `Platform Analytics` و `Live Map Insights` بدلاً من المسميات القديمة، مع تعديل التصميم في `AdminRequests.tsx` لمنع التفاف النص (`whiteSpace: nowrap`).
- تم حل مشكلة TypeScript في `AuthModal.tsx` الخاصة بالـ `GeometryUnion`.
- تم تنظيف مسار المشروع والـ backend من كافة ملفات السكريبتات المؤقتة (`.js`) التي تم استخدامها للـ Debugging والتعديلات لمرة واحدة.
- تمت إضافة معالجة للأخطاء (Error Fallback) في `emailService.js` (تحديداً `sendBookingEmail`) بحيث أنه في حال فشل إرسال إيميل الموافقة/الرفض لعميل بسبب أن الإيميل وهمي أو غير متاح، يتم إرسال رسالة تفصيلية تحتوي على الخطأ للإيميل الخاص بالإدارة المسجل في `.env`.