...
exports.getUsersByRole = async (req, res) => {
  try {
    ...
    if (role === 'owner') {
      ...
      const profiles = await BrokerProfile.find().populate('userId', '-password');
      users = profiles.map(p => ({ ...p.userId.toObject(), profile: p }));
    } else if (role === 'engineer') {
      const profiles = await EngineerProfile.find().populate('userId', '-password');
      users = profiles.map(p => ({ ...p.userId.toObject(), profile: p }));
    } else if (role === 'admin') {
      const profiles = await AdminProfile.find().populate('userId', '-password');
      users = profiles.map(p => ({ ...p.userId.toObject(), profile: p }));
    }
