  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
        <h4 style={{ margin: 0, color: '#f85149' }}>System Admins</h4>
        <input 
          type="text" 
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '20px' }}>
        <thead>
          <tr style={{ backgroundColor: '#21262d', textAlign: 'left' }}>
            <th style={{ padding: '12px', borderBottom: '1px solid #30363d' }}>Admin ID</th>
            <th style={{ padding: '12px', borderBottom: '1px solid #30363d' }}>Name & Contact</th>
            <th style={{ padding: '12px', borderBottom: '1px solid #30363d' }}>Age</th>
        ...
        <tbody>
          {loading ? (
            ...
            <tr><td colSpan={4} style={{ padding: '20px', textAlign: 'center', color: '#8b949e' }}>No admins found.</td></tr>
          ) : (
            filteredUsers.map(user => (
              <tr key={user._id} style={{ borderBottom: '1px solid #30363d' }}>
                <td style={{ padding: '12px', fontWeight: 'bold', color: '#f85149' }}>{user.profile?.manualId}</td>
                <td style={{ padding: '12px' }}>
                  <div style={{ fontWeight: 'bold' }}>{user.name}</div>
                  <div style={{ fontSize: '11px', color: '#8b949e' }}>{user.phone}</div>
                </td>
                <td style={{ padding: '12px', color: '#c9d1d9' }}>{user.profile?.age || 'N/A'} yrs</td>
