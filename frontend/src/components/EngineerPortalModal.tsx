      ...
      <div key={complaint._id} style={{ backgroundColor: complaint.priority === 'High' ? '#2c1214' : 'var(--bg-secondary)', border: `1px solid ${complaint.priority === 'High' ? 'var(--accent-red)' : 'var(--border-color)'}`, borderRadius: '12px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <div style={{ backgroundColor: 'var(--bg-primary)', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '5px' }}>
          {complaint.assignedTechnician ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', paddingBottom: '10px', borderBottom: '1px solid var(--border-color)' }}>
              <div style={{ fontSize: '13px', color: 'var(--text-muted)', fontWeight: 'bold' }}>👷 Current Assignment</div>
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', backgroundColor: 'var(--bg-primary)' }}>
            ...
            {/* Internal / External Toggle Buttons */}
              <div style={{ display: 'flex', padding: '15px', gap: '15px', borderBottom: '1px solid var(--border-color)', backgroundColor: 'var(--bg-primary)' }}>
                <button 
                  onClick={() => setActiveTypeTab('internal')}
                  style={{ flex: 1, padding: '12px', borderRadius: '8px', border: activeTypeTab === 'internal' ? '1px solid var(--accent-blue-bg)' : '1px solid var(--border-color)', backgroundColor: activeTypeTab === 'internal' ? 'var(--accent-blue-bg)22' : 'var(--bg-secondary)', color: activeTypeTab === 'internal' ? 'var(--accent-blue)' : 'var(--text-secondary)', fontWeight: 'bold', cursor: 'pointer', fontSize: '15px', transition: '0.2s' }}
                >
                  🏠 Internal Complaints ({(internalHigh.length + internalNormal.length)})
                </button>
                <button 
                  onClick={() => setActiveTypeTab('external')}
                  style={{ flex: 1, padding: '12px', borderRadius: '8px', border: activeTypeTab === 'external' ? '1px solid #ff7b72' : '1px solid var(--border-color)', backgroundColor: activeTypeTab === 'external' ? '#ff7b7222' : 'var(--bg-secondary)', color: activeTypeTab === 'external' ? '#ff7b72' : 'var(--text-secondary)', fontWeight: 'bold', cursor: 'pointer', fontSize: '15px', transition: '0.2s' }}
                >
              ...
              <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                ...
                {activeTypeTab === 'internal' && (
                  <>
                    <div style={{ flex: 1, borderRight: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', padding: '20px', overflowY: 'auto' }}>
                      <h3 style={{ color: 'var(--accent-red)', margin: '0 0 15px 0', borderBottom: '1px solid var(--border-color)', paddingBottom: '10px' }}>🚨 High Priority ({internalHigh.length})</h3>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        {internalHigh.length === 0 ? <div style={{color: 'var(--text-muted)'}}>No high priority internal complaints.</div> : internalHigh.map(renderComplaintCard)}
                      </div>
                    </div>
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '20px', overflowY: 'auto' }}>
                      <h3 style={{ color: 'var(--text-muted)', margin: '0 0 15px 0', borderBottom: '1px solid var(--border-color)', paddingBottom: '10px' }}>✅ Normal Priority ({internalNormal.length})</h3>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        {internalNormal.length === 0 ? <div style={{color: 'var(--text-muted)'}}>No normal priority internal complaints.</div> : internalNormal.map(renderComplaintCard)}
                      </div>
                    </div>
                  </>
                )}
                ...
                {activeTypeTab === 'external' && (
                  <>
                    <div style={{ flex: 1, borderRight: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', padding: '20px', overflowY: 'auto' }}>
                      <h3 style={{ color: 'var(--accent-red)', margin: '0 0 15px 0', borderBottom: '1px solid var(--border-color)', paddingBottom: '10px' }}>🚨 High Priority ({externalHigh.length})</h3>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        {externalHigh.length === 0 ? <div style={{color: 'var(--text-muted)'}}>No high priority external complaints.</div> : externalHigh.map(renderComplaintCard)}
                      </div>
                    </div>
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '20px', overflowY: 'auto' }}>
                      <h3 style={{ color: 'var(--text-muted)', margin: '0 0 15px 0', borderBottom: '1px solid var(--border-color)', paddingBottom: '10px' }}>✅ Normal Priority ({externalNormal.length})</h3>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        {externalNormal.length === 0 ? <div style={{color: 'var(--text-muted)'}}>No normal priority external complaints.</div> : externalNormal.map(renderComplaintCard)}
                      </div>
                    </div>
                  </>
                )}
